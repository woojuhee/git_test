...
void setLabel(Mat& image, string str, vector<Point> contour)
{
	...
	rectangle(image, pt + Point(0, baseline), pt + Point(text.width, -text.height), CV_RGB(200, 200, 200), cv::FILLED);
...
int main(int, char**)
{
	...
	for (;;)
	{
		...
		adaptiveThreshold(img_temp, img_gray,
			255, ADAPTIVE_THRESH_GAUSSIAN_C, THRESH_BINARY_INV, 201, 7);
		// <-----------------------   parameter 1,  2
...
//영상에서 점과 라인이 인식되면 저장해둔 이미지들이 영상에 같이 출력됨